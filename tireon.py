...
class App:
    ...
    def sampleDataRef(self, coeff):
        ...
        for i in range(0, samples, 1):
            s = self.slip_scale * (i - samples2)
            a = self.slip_angle_scale * (i - samples2) * pi / 180.0
        ...
        fy, dy, bcy, shf = PacejkaFy0(coeff, slip_angle, camber, fz, dfz)
        acp = []
        for i in range(0, samples, 1):
            s = self.slip_scale * (i - samples2)
            fx = 2 * fz * (1 - afx[i][1] / samples2)
            fcx, fcy = PacejkaCombine0(coeff, s, slip_angle, camber, fx, fy, dy, dfz)
            acp.append((samples2 * (1 - 0.5 * fcx / fz), samples2 * (1 - 0.5 * fcy / fz)))
        fy, dy, bcy, shf = PacejkaFy0(coeff, -slip_angle, camber, fz, dfz)
        acn = []
        for i in range(0, samples, 1):
            s = self.slip_scale * (i - samples2)
            ...
            fcx, fcy = PacejkaCombine0(coeff, s, -slip_angle, camber, fx, fy, dy, dfz)
            acn.append((samples2 * (1 - 0.5 * fcx / fz), samples2 * (1 - 0.5 * fcy / fz)))
        return afx, afy, amz, acp, acn
